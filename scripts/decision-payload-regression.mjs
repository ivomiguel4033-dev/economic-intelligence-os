...
try {
  ...
  assert(body.error === "Decision request payload too large", "Oversized decision response must use the bounded-payload error");
} finally {
  await stopServer(child);
}

assert(!/UnhandledPromiseRejection/i.test(stderr), "Decision payload regression server emitted an unhandled rejection");
console.log("Decision payload bound regression checks passed");

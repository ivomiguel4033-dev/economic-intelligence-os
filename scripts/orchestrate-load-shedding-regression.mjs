...
const boundedReadIndex = route.indexOf("await readBoundedPayload(");
const parseIndex = route.indexOf("JSON.parse(payload.payload)");
...
assert.ok(boundedReadIndex > saturationIndex, "load shedding must happen before reading request bodies");
assert.ok(parseIndex > boundedReadIndex, "bounded request bodies must be read before JSON parsing");
